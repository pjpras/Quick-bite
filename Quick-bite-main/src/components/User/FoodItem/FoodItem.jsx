...
const FoodItem = ({id, name, price, description, image, avgRating, category}) => {
    ...
    const removeFromCart = async () => {
        ...
        try {
            ...
            if (newCount === 0) {
                ...
                const response = await api.patch(`/app2/api/v1/cart/update/${id}?quantity=${newCount}`);
    ...
    const getRatingImage = (rating) => {
        ...
        if (rating >= 0 && rating <= 1.20) {
            return assets['1'];
        } else if (rating >= 1.21 && rating <= 1.70) {
            return assets['1.5'];
        } else if (rating >= 1.71 && rating <= 2.20) {
            return assets['2'];
        } else if (rating >= 2.21 && rating <= 2.70) {
            return assets['2.5'];
        } else if (rating >= 2.71 && rating <= 3.20) {
            return assets['3'];
        } else if (rating >= 3.21 && rating <= 3.70) {
            return assets['3.5'];
        } else if (rating >= 3.71 && rating <= 4.20) {
            return assets['4'];
        } else if (rating >= 4.21 && rating <= 4.70) {
            return assets['4.5'];
    ...
    return (
        <div className='food-item'>
            <div className='food-item-img-container'>
                ...
                {itemCount === 0
                    ? <img className='add' onClick={addToCart} src={assets.add_icon_white}/>
            ...
            <div className='food-item-info'>
                ...
                <p className='food-item-category'>{category?.name || 'Uncategorized'}</p>
